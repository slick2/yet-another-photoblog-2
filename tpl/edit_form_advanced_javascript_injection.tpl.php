...
<script language="JavaScript">
	...
	function enhanceForm() {
		...
		if (window.ActiveXObject) $('post').encoding = 'multipart/form-data';
		else $('post').enctype = 'multipart/form-data';

		$('post').acceptCharset = 'UTF-8';
		// Insert the fileupload field
		new Insertion.After($('titlediv'), '<?php echo YapbUtils::escape($this->content) ?>');
	}

	// We can call the function right now, because wordpress already 
	// generated all we need for this.
	// We could also plug this in as onLoad method via prototype.js:
	// Event.observe(window, 'load', enhanceForm, false);
