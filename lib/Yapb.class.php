<?php
	...
	*/
	...
	class Yapb {
		 ...
		 * i want to insert another column after the id column
		...
		function manage_posts_columns($posts_columns) {
			...
			foreach ($posts_columns as $key => $value) {
				if ($key == 'id') {
					$result[$key] = $value;
					$result['thumb'] = __('Thumbnails', 'yapb');
				...
				if (is_feed()) {
					...
					if (get_option('yapb_display_images_xml')) {
						...
						if (get_option('yapb_display_images_xml_thumbnail_activate')) {
							$options = array('w=' . get_option('yapb_display_images_xml_thumbnail'));
							$embed .= 
								$post->image->getThumbnailHref(
									$options, 
									false // I manually override the users xhtml setting for WordPress feeds: Have a look above
								);
							$embed .= '" ';
